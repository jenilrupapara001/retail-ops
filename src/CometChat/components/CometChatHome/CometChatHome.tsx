...
function CometChatHome({
  ...
  useEffect(() => {
    ...
    const ccGroupMemberUnbanned = CometChatGroupEvents.ccGroupMemberUnbanned.subscribe(() => {
  ...
  useEffect(() => {
    if (layoutFeatures && layoutFeatures?.tabs && layoutFeatures?.withSideBar) {
      ...
      // On first run, prefer defaultActiveTab if provided and valid
      if (!hasInitializedTab.current && defaultActiveTab && layoutFeatures?.tabs?.includes(defaultActiveTab)) {
        tabToSet = defaultActiveTab;
      }
      // On subsequent runs, check if current activeTab is still valid
      else if (hasInitializedTab.current && layoutFeatures?.tabs?.includes(activeTab)) {
        // Current tab is still valid, keep it
        return;
      }
      // Fallback to priority order if defaultActiveTab is invalid or current tab was removed
      else if (layoutFeatures?.tabs?.includes('chats')) {
        tabToSet = 'chats';
      ...
      } else if (tabToSet === 'calls' && loggedInUser) {
        fetchDefaultCallDetail();
      } else if (tabToSet === 'users') {
        fetchDefaultUser();
      } else if (tabToSet === 'groups') {
        fetchDefaultGroup();
      }
